                ...
                <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="relative">
                            ...
                            <svg className="pointer-events-none absolute right-2 top-1/2 h-3 w-3 -translate-y-1/2" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        {workouts.map((workout) => (
                            <div key={workout.id} className="flex min-h-[114px] flex-col gap-4 rounded-xl bg-[#13161D] p-4 min-[768px]:h-[114px] min-[768px]:flex-row min-[768px]:items-center min-[768px]:gap-0">
                                ...
                                <div className="flex w-full flex-wrap items-center gap-1.5 min-[768px]:w-auto min-[768px]:flex-nowrap min-[768px]:gap-2">
                                    ...
                                    {activeTab === "today" && (
                                        <button onClick={() => { removeFromPlan(workout.id); toast.success("Workout marked as done") }} className="shrink-0 cursor-pointer rounded-full bg-[#C2F800] px-3 py-1.5 font-[family-name:var(--font-inter)] text-[11px] font-bold text-[#0F1115] sm:px-4 sm:py-2 sm:text-[12px]">✓ Mark as Done</button>
                                    )}
                                    <button onClick={() => { if (activeTab === "today") { removeFromPlan(workout.id); toast.success("Removed from today's plan") } else { removeSaved(workout.id); toast.success("Removed from saved") } }} className="cursor-pointer px-1.5 text-lg text-white sm:px-2 sm:text-xl">×</button>
                                </div>
                            </div>
                        ))}
                    </div>
