...
export default function WorkoutCard({ workout }: { workout: Workout }) {
    return (
        <Link href={`/workouts/${workout.id}`} className="block">
            <div className="cursor-pointer h-[368px] w-full overflow-hidden rounded-3xl bg-[#222630]">
                <Image src={workout.image} alt={workout.name} width={394} height={192} className="h-[192px] w-full object-cover" />
                <div className="px-5 py-4">
                    ...
                    <div className="flex items-center gap-5 font-[family-name:var(--font-inter)] text-[14px] text-[#9CA3AF]">
                        ...
                        <span className="flex items-center gap-1.5"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>{workout.rating}</span>
                    </div>
                </div>
            </div>
        </Link>
